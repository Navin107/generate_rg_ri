...
route = '6e9d6c91-7885-4faa-bb61-2406bb2bd354/.949b2ee8-d058-4f4b-a267-3f21e4ec4515'
ri_uuid = "949b2ee8-d058-4f4b-a267-3f21e4ec4515"
...
class GandhinagarATCS(object):
    ...
    def transformData(self, json_data):
        ...
        for packet in json_array:
            ...
            final_packet["location"] = {
                                        "type": "point",
                                        "coordinates": [float(packet["longitude"]), float(packet["latitude"])]
                                       }
    ...
    def deDuplication(self, current_list_of_packets):
        ...
        if not(path.exists('../misc/gandhinagar-atcs-junction.json')):
            with open('../misc/gandhinagar-atcs-junction.json', 'w', encoding='utf-8') as fp:
                    
                    json.dump(current_list_of_packets, fp, indent=6)
                    self.publish_data(current_list_of_packets)

        else:
            with open('../misc/gandhinagar-atcs-junction.json', 'r+', encoding='utf-8') as fp:
                    json_str = fp.read()
